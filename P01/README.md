## Project 1: SQL Assignment, CMSC424, Spring 2017
...
1. The databases and users are all set up for you when the VM is instantiated.
1. You do not need a token to use the Python notebooks: always go to
   'http://localhost:8889' (note this is a different port than Project0).
...
- If you are unable to use vagrant/virtualbox, I can set up a VM on one of our
  servers. This will have the same ubuntu release ("xenial"), but you will
  need to install the packages and do setup yourself. Pretty much all of this
  is self-explanatory at the end of the *Vagrantfile*.
...
This will start a server on the VM, listening on port 8889. We will access it
	from the **host** (as discussed in class, the VagrantFile maps the 8888 port
	on the guest VM to the 8888 port on the host VM). To do that, simply start
	the browser, and point it to: http://localhost:8889 (note we no longer
