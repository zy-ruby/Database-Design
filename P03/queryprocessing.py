...
class GroupByAggregate(Operator):
	...
	def initial_value(aggregate_function):
		initial_values = [0, 0, None, None, [0,0], [0,[]], {}]
	...
	def update_aggregate(aggregate_function, current_aggregate, new_value):
		if aggregate_function == GroupByAggregate.COUNT:
			...
			current_aggregate[0] += 1 #count
			current_aggregate[1] += float(new_value) #sum
			return current_aggregate

		elif aggregate_function == GroupByAggregate.MEDIAN:
			current_aggregate[0] += 1 #count
			current_aggregate[1].append(new_value) #
			return current_aggregate
		elif aggregate_function == GroupByAggregate.MODE:
			if current_aggregate.has_key(int(new_value)):
				count_temp = int(current_aggregate.get(int(new_value)))
				count_temp += 1
				current_aggregate[int(new_value)] = count_temp
			else:
				current_aggregate.setdefault(int(new_value),1)
			return current_aggregate
	...
	def final_aggregate(aggregate_function, current_aggregate):
		if aggregate_function in [GroupByAggregate.COUNT, GroupByAggregate.SUM, GroupByAggregate.MIN, GroupByAggregate.MAX]:
			...
			current_aggregate[1].sort()
			if len(current_aggregate[1]) % 2 == 1:
				return current_aggregate[1][current_aggregate[0]/2]
			else:
				m1 = int(current_aggregate[1][current_aggregate[0]/2])
				m2 = int(current_aggregate[1][(current_aggregate[0]/2)-1])
				return (m1+m2)/2
		elif aggregate_function == GroupByAggregate.MODE:
			dict = sorted(current_aggregate.iteritems(), key=lambda d: d[1], reverse=True)
			return dict[0][0]
		else:
			raise ValueError("No such aggregate")
...
class SortMergeJoin(Operator):
	...
	def __init__(self, left_child, right_child, left_attribute, right_attribute, jointype = INNER_JOIN):
		self.left_child = left_child#left_child is a schema
		self.right_child = right_child
		self.left_attribute = left_attribute
		self.right_attribute = right_attribute
		self.jointype = jointype
	...
	def get_next(self):
		left_input = [r for r in self.left_child.get_next()]#left_input is a list[tuple]
		right_input = [r for r in self.right_child.get_next()]

		left_input.sort(key=lambda t: t.getAttribute(self.left_attribute))# t is a tuple
		...
		if self.jointype == self.INNER_JOIN:
			...
			while ptr_l < len(left_input) and ptr_r < len(right_input):
				set_L = [left_input[ptr_l]]#set_L is a list[tuple]
			...
			while ptr_l < len(left_input) and ptr_r < len(right_input):
				...
				if ptr_l < len(left_input) and ptr_r < len(right_input) and right_input[ptr_r].getAttribute(self.right_attribute) > l_attr:
					for l in set_L:
						output = list(l.t)
						r_none = []
						for i in range(0, len(self.right_child.relation.schema), 1):
							r_none.append("NULL")
						output.extend(r_none)
						yield Tuple(None, output)



				elif  ptr_r < len(right_input) and right_input[ptr_r].getAttribute(self.right_attribute) <= l_attr:
					while ptr_r < len(right_input) and right_input[ptr_r].getAttribute(self.right_attribute) <= l_attr:
						if right_input[ptr_r].getAttribute(self.right_attribute) == l_attr:
							for l in set_L:  # l is a tuple
								output = list(l.t)
							...
							l_none = []
							for j in range(0, len(self.left_child.relation.schema), 1):
						 		l_none.append("NULL")
							output = l_none
							output.extend(list(right_input[ptr_r].t))
							yield Tuple(None,output)
			...
			while ptr_l < len(left_input):
				output = list(list(left_input[ptr_l].t))
				r_none = []
				for i in range(0, len(self.right_child.relation.schema), 1):
					r_none.append("NULL")
				output.extend(r_none)
...
class SetMinus(Operator):
	def __init__(self, left_child, right_child, keep_duplicates ):
		self.left_child = left_child
		self.right_child = right_child
	...
	def get_next(self):
		...
		hashtable = dict()
		for l in self.left_child.get_next():
			if str(l) in hashtable:
				if self.keep_duplicates == True:
					hashtable[str(l)].append(l)
					# print "--now append--"+str(l)
			else:
				hashtable[str(l)] = [l]
				# print "--now start--"+str(l)
			# print'-----hashtable[l]----'
			# print str(l) + ":" + str(hashtable[str(l)])
		# Then, for each tuple in the right input, we look for matches and output those
		# Using "yield" significantly simplifies this code
		for r in self.right_child.get_next():
			if str(r) in hashtable:
				del hashtable[str(r)][0]
				# print "---now remove----"+str(r)
				if len(hashtable[str(r)]) == 0:
					del hashtable[str(r)]

		for key in hashtable:
			while len(hashtable[key]) != 0:
				tuple = hashtable[key].pop()
				output = tuple.t
				yield Tuple(None, output)
		# raise ValueError("Functionality to be implemented")
		
	# Typically you would close any open files e
	# tc.
	def close(self):
		return
