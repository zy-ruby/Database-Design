...
class BTreeBlock(Block):
    ...
    def findSiblingWithSameParent(self, parentBlock):
        ...
        for index in range(0, len(parentBlock.keysAndPointers), 2):
            # print "findSiblingWithSameParent:  "+ str(self.blockNumber)
    ...
    def mergeEntriesFromBlock(self, otherBlock, key = None):
        ...
        if self.isLeaf:
            # print "merge leaf entries from block:  "+str(self.blockNumber)+"  " +str(otherBlock.blockNumber)
            ...
            # print "merge nonleaf entries from block:  "+str(self.blockNumber)+"  " +str(otherBlock.blockNumber)
    ...
    def redistributeWithBlock(self, otherBlock):
        ...
        if otherBlock.isUnderfull(): # In pseudocode: if (N' is a predecessor of N)
            ...
            if not self.isLeaf:
                # print " self->other: non leaf block redistributed with sibling:  " + str(self.blockNumber) +"->"+ str(otherBlock.blockNumber)
                ...
                otherBlock.keysAndPointers.insert(0,parentBlock.keysAndPointers[index])
                otherBlock.keysAndPointers.insert(0,pm)

                # Remove the last pointer; then remove and return the last key
                return km1
            else:
                # print "self->other:leaf block redistributed with sibling:  " + str(self.blockNumber) + "->" + str(
            ...
            if not self.isLeaf:
                # print "self<-other:non leaf block redistributed with sibling:  " + str(self.blockNumber) + "<-" + str(
                #     otherBlock.blockNumber)
                parentBlock = self.parent.getBlock()
                for i in range(0,len(parentBlock.keysAndPointers),2):
                    if parentBlock.keysAndPointers[i].blockNumber == otherBlock.blockNumber:
                        index = i-1

                p1 = otherBlock.keysAndPointers.pop(0)
                k1 = otherBlock.keysAndPointers.pop(0)

                self.keysAndPointers.append(parentBlock.keysAndPointers[index])
